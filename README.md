...
### 목업 데이터 테스트 모드 (Cursor Admin API 형태 기반)

실제 Enterprise API 키 없이 UI/알림 흐름을 테스트하려면 설정에서 API Key를 아래 값으로 저장하세요.

```text
mock_demo
```

저장 후 수동 새로고침 또는 자동 동기화가 실행되면 다음 엔드포인트 형식에 맞춘 목업 데이터가 로드됩니다.

- `POST /teams/spend`
- `POST /teams/daily-usage-data`
- `POST /teams/user-spend-limit` (관리자 한도 설정 테스트 응답)
